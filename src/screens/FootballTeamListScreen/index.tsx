...
import { fetchFootballTeams, loadMoreFootballTeams } from "@store"
...
export const FootballTeamListScreen = ({ navigation }: TRootScreenProps<EScreens.FootballTeamListScreen>) => {
    ...
    const onPressTeamItem = useCallback((teamId: number) => {
        navigation.navigate(EScreens.FootballDetailScreen, { id: teamId })
    }, [navigation])
                ...
                renderItem={({ item, index }) => {
                    return (
                        <FootballTeamListItem
                            ...
                            logoUrl={item.crest}
                            name={item.name}
                            onPress={() => onPressTeamItem(item.id)}
                        />
                    )
                }}
